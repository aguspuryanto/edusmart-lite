...
import { Plus, Search, FileText, Download, Eye, EyeOff, Loader2, BookOpen, School } from 'lucide-react';
...
export default function Materials() {
  ...
  const handleAddMaterial = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setFormData({ title: '', content: '', subject_id: '', class_id: '', is_draft: false });
        setShowModal(false);
        fetchData();
      }
    } catch (err) {}
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Akses dan kelola materi belajar Anda.</p>
        </div>
        {user?.role !== 'student' && (
          <button
        ...
        {loading ? (
          ...
          materials.map((m, i) => (
            ...
            >
              ...
              <p className="text-xs text-slate-500 mb-4 line-clamp-2">{m.content || 'Tidak ada deskripsi'}</p>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center text-[10px] font-bold text-slate-500">
                    {m.teacher_name.charAt(0)}
                  </div>
                  <span className="text-[10px] font-medium text-slate-500">{m.teacher_name}</span>
                </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          ...
          >
            <h3 className="text-xl font-bold text-slate-900 mb-6">Tambah Materi Baru</h3>
            <form onSubmit={handleAddMaterial} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Deskripsi/Konten</label>
                <textarea
                  value={formData.content}
                  onChange={(e) => setFormData({ ...formData, content: e.target.value })}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all min-h-[120px]"
                  placeholder="Tulis deskripsi atau konten materi..."
                />
              </div>
